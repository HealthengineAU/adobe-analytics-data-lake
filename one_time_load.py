...
args = {'s3target': 's3://ingram-gbi3-adobe-analytics-data-lake/hit-data/', 's3source': 's3://ingram-gbi-adobe-analytics-data-feed', 'JOB_NAME': 'adobe-analytics-data-feed-to-lake'}
...
response = client.list_objects_v2(Bucket = s3sourcebucket, Prefix = 'incoming_2019_01_01__2019_07_10/')
...
batch_size = 1
while i < len(files):
    file = files[i: i + batch_size]
    print('loading files')
    for f in file:
        print(f)
    df0 = spark.read.format("com.databricks.spark.csv").option("quote", "\"").option("delimiter", u'\u0009').option("charset", 'utf-8').schema(schema).load(file)
